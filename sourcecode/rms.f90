PROGRAM rms
  ...
  INTEGER :: ndim, startvec, endvec, natom, j, i, io, numvec, vector, k, ncalpha, nexp
...
! Assigning atom masses
  ...
  WRITE(2357,'(A)',advance='NO') '#   Num At Res Chain Resid '
  DO i=1,numvec
  ...
  DO i=1,natom
     ...
     IF (nexp.eq.0.and.name(i).eq.' CA ') THEN
  ...
  IF (ncalpha.gt.0) THEN
     ...
     WRITE(6745,'(A)')'#    Number Name Res Chain Num   Predicted   FullScaled    Scaled    Experimental'

     DO i=1,natom
        WRITE(6745,'(A6,I5,1X,A4,1X,A3,1X,A1,I4,1X,F16.4,3F12.4)') atom(i),atnum(i),name(i),res(i),chain(i),resnum(i), &
...
SUBROUTINE helptext(iunit)
  ...
  WRITE(iunit,'(A)')"from the included ENM code, GNM code, or the GroAMED package.            "
  WRITE(iunit,'(A)')"                                                                         "
  ...
  WRITE(iunit,'(A)')"  -het      Opt                        Incudes reading HETATM records    "
  WRITE(iunit,'(A)')"                                                                         "

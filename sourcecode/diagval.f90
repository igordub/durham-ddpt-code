...
! sort d and a in order of eigenvalue
  ...
  CALL write_eigenvals(outfile,a,d,3*natom,3*natom)
...
SUBROUTINE helptext(iunit)
  IMPLICIT NONE
  INTEGER :: iunit
  WRITE(iunit,'(A)')"                           D  I  A  G  V  A  L                           "
  WRITE(iunit,'(A)')"                               VERSION 1.0                               "
  WRITE(iunit,'(A)')"                                                                         "  
  WRITE(iunit,'(A)')"                               Written by:                               "
  WRITE(iunit,'(A)')"                            Igors Dubanevics                             "
  WRITE(iunit,'(A)')"                                                                         "
  ...
  WRITE(iunit,'(A)')"A matrix.eigenvals file is produced which can be further analysed using  "
  WRITE(iunit,'(A)')"the programs in this toolbox. Can only be used by FREQEN.                "
  WRITE(iunit,'(A)')"Based on DIAGSTD module. Writes only eigenvalues and ommits egeinvectors."
  WRITE(iunit,'(A)')"                                                                         "
  WRITE(iunit,'(A)')"     Usage:                                                              "
  WRITE(iunit,'(A)')"             diagval [-i matrix]                                         "
  WRITE(iunit,'(A)')"                                                                         "

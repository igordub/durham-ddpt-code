####--------------------------------------------------------------------
####-----------  H I N G E F I N D  - TCL PLUG-IN FOR VMD --------------
####--------------------------------------------------------------------
#   This script has been produced by Tom Rodgers 2011 and is based on
#   the original scrip by Willy Wriggers and thus retains his original
#   copyright, (c) 1995-1997, by Willy Wriggers 
...
#
#### LOAD -------------------------------------------------------------- 
proc hingeload {pdb1 pdb2 {plottype "Tube"} } {
...
#     at the beginning of a session.
    ...
    set full1 [atomselect $mol1 "name CA"]
    $full1 global
    set ca1 $full1

    # select the compared atoms in the second molecule 
    set full2 [atomselect $mol2 "name CA"]
    $full2 global
    set ca2 $full2
...
#### PARTITION --------------------------------------------------------- 
proc partition {eps {plottype "Tube"} } {
...
#### FINDRMS ----------------------------------------------------------- 
proc findrms {} {
#   - Calculates the rms between to two molecules
#   - called by the user.

    global mol1 mol2

    set cas1 [atomselect $mol1 "name CA"]
    set cas2 [atomselect $mol2 "name CA"]

    set cacord1 [$cas1 get {x y z}]
    set cacord2 [$cas2 get {x y z}]
    set caindex [$cas1 get resid]

    set fo [open "./RSplot.dat" "w"]
    
    foreach cd1 $cacord1 cd2 $cacord2 ind $caindex {
        set dist [vecdist $cd1 $cd2]
	puts $fo "$ind $dist"
    } 
    close $fo
